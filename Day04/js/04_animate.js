$(function(){
    ...
    /*
        hide(인자1, 인자2)
        * 인자1 : duration(시간)
        - margin, padding, width, height, opacity 를 지정한 값에서 0으로 애니메이션
    ...
    */
    ...
    $('#item1 .hide1000').on('click', function(){
        ...
        // - function() {}
        // - () => {}
       $('#item1 .target-box').hide(1000, function(){
            alert('hide')
        })
    })
    ...
    */
    ...
    //slideDown 버튼 클릭 이벤트
    $('#item3 .slieDown').on('click', function(){
        $('#item3 .target-box').slideDown()
    })

    //slideUp 버튼 클릭 이벤트
    $('#item3 .slieUp').on('click', function(){
        $('#item3 .target-box').slideUp()
    })
    
    //slideToggle 버튼 클릭 이벤트
    $('#item3 .slieToggle').on('click', function(){
    ...
    /*
        ...
        1. 이벤트 정의
            ex) click 이벤트
        2. 애니메이션 적용 될 요소에 'active' 라는 클래스 추가/삭제 지정
            - toggleClass('active')
        3. CSS에서 .active에 애니메이션 적용할 스타일 속성
